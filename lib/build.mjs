...
//
// WHICH BUNDLE GETS PATCHED — read this before building
// ----------------------------------------------------
// Several DSH installs can live side by side on one machine, each with its own
// DSH_HOME (e.g. `~/.dsh` on core 0.1.1-rc.2 and `~/.dsh-next` on 0.1.5-rc.1).
// This script patches whichever install it resolves, and the patched client is
// only valid against THAT core version: a bundle built from an older core's
// ui-workspace does not activate on a newer core, and the failure mode is the
// worst possible one — the whole Web GUI comes up blank (the module loader
// reports "N entries did not activate … waiting for service: uiWorkspace"),
// not merely a missing panel.
//
// The patch anchors below are version-checked substrings, but they are checked
// only for PRESENCE: an older official bundle can still satisfy every anchor and
// build cleanly. So anchor success does NOT mean "built against the right core".
//
// Therefore:
//   * `DSH_HOME` selects the install, exactly like `dsh web` reads it:
//       DSH_HOME="$HOME/.dsh-next" node lib/build.mjs
//   * Without it, the default install is used and a warning lists any other
//     installs found, so a mismatched build is hard to make by accident.
//   * The resolved bundle's own package version is printed for the same reason.
...
/** The `@deepseek-ai/dsh-client-ui-workspace` client bundle inside one DSH home. */
const bundleIn = (dshHome) => join(dshHome, "profiles", "web", "node_modules", "@deepseek-ai", "dsh-client-ui-workspace", "lib", "client.js")
const fallbackBundleIn = (dshHome) => join(dshHome, "profiles", "node_modules", "@deepseek-ai", "dsh-client-ui-workspace", "lib", "client.js")

const envHome = process.env.DSH_HOME
const defaultHome = join(homedir(), ".dsh")
// DSH_HOME first (profile-local install, then the shared fallback dir), then the
// default install, then any npx-store copy.
const candidates = [
  ...(envHome ? [bundleIn(envHome), fallbackBundleIn(envHome)] : []),
  bundleIn(defaultHome),
  fallbackBundleIn(defaultHome),
...
/** The `@deepseek-ai/dsh-client-ui-workspace` version a resolved bundle belongs to. */
const versionOf = (bundlePath) => {
  try {
    const pkg = JSON.parse(readFileSync(join(dirname(bundlePath), "..", "package.json"), "utf8"))
    return `${pkg.name}@${pkg.version}`
  } catch {
    return "(unknown version — no readable package.json beside the bundle)"
  }
}

console.log(`patching: ${SRC}`)
console.log(`          ${versionOf(SRC)}`)

if (envHome === undefined) {
  // Warn when another DSH home exists, because building against the wrong core is
  // silent here and blank-GUI at runtime.
  const others = [
    join(homedir(), ".dsh-next"),
    join(homedir(), ".dsh-rc"),
    join(homedir(), ".dsh-test"),
  ].filter((h) => h !== defaultHome && (existsSync(bundleIn(h)) || existsSync(fallbackBundleIn(h))))
  if (others.length > 0) {
    console.warn(`warning: DSH_HOME is unset, so the default install (${defaultHome}) was used.`)
    console.warn(`         another DSH install was found at: ${others.join(", ")}`)
    console.warn(`         if you run that one, rebuild with: DSH_HOME=<path> node lib/build.mjs`)
    console.warn(`         (a client built from a different core version renders a blank GUI)`)
  }
}
...
src = src.replace(OLD_ID, `id: "@lim324/dsh-worktree-panel",`)
...
const NEW_RETURN = `			const wtpRepoByWorkspace = new Map();
			...
			//
			// Hiding a worktree unconditionally therefore strands every session of its
			// workspace with nothing left to render it: the row vanishes on the first
			// topology reply and never comes back, so those sessions become unreachable
			// rather than relocated — which happens whenever the parent project group is
			// collapsed or has no group at all. Only hide a worktree when its replacement
			...
			for (const repo of wtp.tree.repos) {
				...
				if (wtpHome === void 0 || wtpHome.expanded !== true) continue;
									...
									children: [
										...
										repo !== void 0 && group.expanded && (function () {
											...
											for (const s of (repo.sessions || [])) {
												...
												if (live !== void 0 && blankOk(live) && !seen.has(live.id)) { seen.add(live.id); all.push(sessionNode(live, wtpDescendants, pendingInteractions)); }
											}
										...
										repo !== void 0 && group.expanded && (repo.worktrees || []).map((wt) => {
											...
											const wtNodes = (wt.sessions || [])
												...
												.map((n) => sessionNode(n, wtpDescendants, pendingInteractions));
											return (0, react_jsx_runtime.jsx)(__wtpWorktreeRow, {
												name: wt.name,
												pill: null,
												dirty: wt.dirty,
...
// ---------------------------------------------------------------------------
// 7. worktree slash commands: register /wtfix & /wtfeat via ctx.inputTriggers.
//    Picked (menu) or space/enter consumes the /wtfix|/wtfeat token and splices
//    "创建 worktree 修复/实现：" so the trailing task text is preserved.
// ---------------------------------------------------------------------------
const SLASH_ANCHOR = `			}, __wtpLocationRow));
		}`
expect(src, SLASH_ANCHOR, "apply ending (settings.general.item)")
src = src.replace(SLASH_ANCHOR, `			}, __wtpLocationRow));
...
// ---------------------------------------------------------------------------
// 8. the subagent-descendant helper moved out of the deleted runtime package.
//
// Patch point 3's injected code calls `indexSubagentDescendants`, which the OLD
// official bundle re-exported from `@deepseek-ai/dsh-client-runtime/client` — a
// package DSH deleted in 0.1.2-alpha. The NEW official bundle defines the same
// helper as a module-local function instead (both bundles have carried the
// implementation all along; only the binding changed).
//
// Without this rewrite the injected code compiles but the identifier does not
// exist at runtime, so rendering the sidebar throws:
//     ReferenceError: _deepseek_ai_dsh_client_runtime_client is not defined
//     slot entry crashed in 'sidebar.workspaces'
// — which leaves the workspace tree EMPTY (only "New Session" renders). It is a
// slot-render crash rather than a load failure, so the GUI still comes up and
// the slash commands still register: easy to miss.
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// 8b. `sessionNode` gained a third parameter in 0.1.5-rc.1.
//
// The official signature is now `sessionNode(s, descendants, pendingInteractions)`
// and the body reads `pendingInteractions.get(s.id)`. Patch point 3/4's injected
// worktree rows called it with two arguments, so `pendingInteractions` was
// `undefined` and rendering a worktree row threw:
//     TypeError: Cannot read properties of undefined (reading 'get')
//     at sessionNode
//     slot entry crashed in 'sidebar.workspaces'
// — which blanks the ENTIRE sidebar, and only on the parent-group-expanded path.
// The calls above now pass the third argument; this guard fails loudly if the
// official signature moves again instead of silently emitting a broken bundle.
// ---------------------------------------------------------------------------
const SESSION_NODE_SIG = "function sessionNode(s, descendants, pendingInteractions)"
if (!src.includes(SESSION_NODE_SIG)) {
  throw new Error(
    `patch anchor not found: official sessionNode signature is not "${SESSION_NODE_SIG}" — ` +
    `re-check the arguments the injected worktree rows pass to sessionNode()`,
  )
}

const DESCENDANTS_OLD = "_deepseek_ai_dsh_client_runtime_client.indexSubagentDescendants"
const DESCENDANTS_LOCAL = "indexSubagentDescendants"
if (src.includes(DESCENDANTS_OLD)) {
  if (!src.includes(`function ${DESCENDANTS_LOCAL}(`)) {
    throw new Error(
      `patch anchor not found: "${DESCENDANTS_OLD}" is referenced but the bundle defines no local ` +
      `${DESCENDANTS_LOCAL}() to bind it to — the helper moved again; re-check this patch point`,
    )
  }
  src = src.replaceAll(DESCENDANTS_OLD, DESCENDANTS_LOCAL)
}
