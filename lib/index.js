...
async function sessionMap(ctx) {
  ...
  // 持久化会话：ctx.sessions.list() 只含当前进程的 live 会话，历史会话在磁盘上。
  // 用 sessionQuery 把持久化会话也并进来（live 优先），让 tree 能显示未加载的会话。
  try {
    const records = await ctx.sessionQuery.listSessions()
    const persistedIds = []
    for (const rec of records) {
      if (rec.live) continue // live 会话上面已处理
      if (byId.has(rec.header.id)) continue
      if (!rec.header.cwd) continue
      persistedIds.push(rec.header.id)
    }
    const titleById = new Map()
    if (persistedIds.length > 0) {
      try {
        const snapshots = await ctx.sessionQuery.readTitleSnapshots(persistedIds)
        for (const snap of snapshots) {
          if (snap.status === "fulfilled" && snap.value.title?.title) {
            titleById.set(snap.sessionId, snap.value.title.title)
          }
        }
      } catch {
        /* 标题读取失败时回退到 id */
    ...
    for (const rec of records) {
      ...
      const summary = { id, title: titleById.get(id) || id, running: false }
      byId.set(id, summary)
      const key = await canonical(cwd)
      const arr = byCwd.get(key) ?? []
      arr.push(summary)
    ...
    /* sessionQuery 不可用时退化为 live-only（保持原行为） */
  }
  return { byCwd, byId }
}
