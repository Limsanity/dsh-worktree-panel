...
const MANIFEST_PATH = join(process.env.DSH_HOME || join(homedir(), ".dsh"), "worktree-panel.json")
...
// 本机 PATH 有 30+ 条（Android SDK / flutter / orbstack / cryptex…），实测每次 spawn
// 120ms，而用绝对路径只要 13ms。解析一次并缓存（一次 PATH 扫描），之后每次 execFile
// 都直接 execve 绝对路径。main 分支本来有这段优化，adapt 重写时丢失了。
let GIT = null
...
async function git(args, cwd) {
  try {
    const { stdout } = await execFileAsync(GIT ?? (GIT = resolveGitPath()), args, {
      cwd,
      timeout: 30000,
...
async function loadManifestSeeded(ctx, sessions) {
...
// 标题 fold 需要读完整会话日志（本机 50 个会话全量解码约 8s），是 /tree 首请求慢的
// 主因。持久化标题缓存：每个会话只 fold 一次，跨请求、跨重启复用；live 会话每次取最新。
const TITLE_CACHE_PATH = join(process.env.DSH_HOME || join(homedir(), ".dsh"), "worktree-panel-titles.json")
let titleCache = null
...
async function sessionMap(ctx) {
  ...
  // 标题：live 会话每次取最新（直接折，无持久化读）；持久化会话只在缓存未命中时 fold。
  const cache = await loadTitleCache()
  const toFold = []
  for (const rec of records) {
    if (isSubagentSession(rec.header)) continue
    if (!rec.header.cwd) continue
    if (rec.live || !cache.has(rec.header.id)) toFold.push(rec.header.id)
  }
  if (toFold.length > 0) {
    try {
      const snapshots = await ctx.sessionQuery.readTitleSnapshots(toFold)
      for (const snap of snapshots) {
        if (snap.status !== "fulfilled") continue
        cache.set(snap.sessionId, snap.value.title?.title ?? null)
      }
  ...
  for (const rec of records) {
    ...
    const summary = { id, title: cache.get(id) || id, running }
    byId.set(id, summary)
...
async function buildTree(ctx) {
  ...
  const manifest = await loadManifestSeeded(ctx, sessions)
